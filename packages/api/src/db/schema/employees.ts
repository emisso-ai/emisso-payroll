...
import { afpProviderEnum } from "./reference-data";
 ...
 */
export const employees = payrollSchema.table(
  "employees",
  {
    ...
    // ── AFP (Pension) ──
    afpCode: afpProviderEnum("afp_code").notNull(),
  ...
  (table) => ({
    tenantActiveIdx: index("pr_employees_tenant_active_idx").on(
      table.tenantId,
      table.isActive,
    ),
    uniqueTenantRut: unique("pr_employees_tenant_rut_unq").on(
      table.tenantId,
      table.rut,
    ),
  }),
